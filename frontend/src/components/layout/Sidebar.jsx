...
export const Sidebar = ({ isCollapsed, setIsCollapsed }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 bottom-0 left-0 z-30 flex flex-col bg-slate-900 border-r border-slate-800 transition-all duration-350 hidden md:flex ${
          isCollapsed ? 'w-20' : 'w-20 lg:w-64'
        }`}
      >
        {/* Brand logo header */}
        <div className={`flex items-center h-16 border-b border-slate-800/80 transition-all ${
          isCollapsed ? 'px-4 justify-center' : 'px-4 justify-center lg:px-6 lg:justify-between'
        }`}>
          <NavLink to="/dashboard" className="flex items-center gap-2.5 group flex-shrink-0">
            ...
            {!isCollapsed && (
              <span className="hidden lg:inline font-bold text-lg bg-gradient-to-r from-indigo-200 via-indigo-50 to-cyan-150 bg-clip-text text-transparent tracking-tight">
                DevTrackr
              </span>
            )}
          </NavLink>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3.5 px-3 py-3 rounded-xl transition-all duration-200 text-sm font-medium ${
                    isCollapsed ? 'justify-center pl-3' : 'justify-center lg:justify-start pl-3 lg:pl-[8.5px]'
                  } ${
                    isActive
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/10 space-y-4">
          ...
          >
            <div className="flex items-center gap-3 pl-1">
              ...
              {!isCollapsed && <span className="hidden lg:inline">{isDark ? 'Light Mode' : 'Dark Mode'}</span>}
            </div>
          </button>
          ...
          {user && (
            <div className={`flex items-center gap-3 p-2 rounded-xl bg-slate-950/30 border border-slate-850 transition-all ${
              ...
              {!isCollapsed && (
                <button
                  onClick={logout}
                  className="hidden lg:flex text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 p-1.5 rounded-lg transition-all"
                  title="Log out"
                >
