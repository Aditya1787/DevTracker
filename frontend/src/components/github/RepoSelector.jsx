...
export const RepoSelector = ({ isCollapsed = false }) => {
  const { repos, selectedRepo, selectRepo } = useContext(GitHubContext);
  ...
  return (
    <div>
      {/* Collapsed state (shown if forced isCollapsed, or on md screens if not lg) */}
      <div className={`${isCollapsed ? 'block' : 'block lg:hidden'} flex justify-center`}>
        <button
          onClick={() => navigate('/repositories')}
          className="p-2.5 rounded-xl bg-slate-800 border border-slate-700/60 hover:bg-slate-750 text-indigo-400 transition-all active:scale-95"
          title={selectedRepo ? `Active: ${selectedRepo.repoName}` : 'Manage Repositories'}
        >
          <GitBranch className="w-5 h-5" />
        </button>
      </div>

      {/* Expanded state (hidden if forced isCollapsed, and hidden on md screens, only shown on lg) */}
      <div className={`${isCollapsed ? 'hidden' : 'hidden lg:block'} relative`}>
